	...
	: projection(glm::perspective(glm::radians(45.0f), 800.0f / 600.0f, 0.1f, 100.0f))
{
	...
	woodTexture.load("texture/diffuseMap.png");
	woodSpecularTexture.load("texture/diffuseMapSpecular.png");
}
...
void Renderer::render(float currentTime, const Camera &camera)
{
	...
	spLight.setuniform("material.diffuse", 0);
	spLight.setuniform("material.specular", 1);
	spLight.setuniform("material.shininess", material.shininess);

	woodTexture.bind(0);
	woodSpecularTexture.bind(1);

	glDrawElements(GL_TRIANGLES, 36, GL_UNSIGNED_INT, 0);

	

	const glm::vec3 wallPositions[] ={
		{0.0f, -5.0f, 0.0f},
		{0.0f, 5.0f, 0.0f},
		{-5.0f, 0.0f, 0.0f},
		{5.0f, 0.0f, 0.0f},
